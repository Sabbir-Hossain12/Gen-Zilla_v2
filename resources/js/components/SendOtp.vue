...
async function handleSendOtp() {
    ...
    } else if (localPhone.value.length < 11 || localPhone.value.length > 11) {
        alert('Number should be 11 digits')
    } else {
        auth.phone = localPhone.value;
        await auth.sendOtp();
    }
...
<template>
    <Teleport to="body">
        <div v-if="auth.showSendOtpModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4"
             @click.self="close">
            <div id="loginModal"
                ...
                <div class="relative bg-white rounded-2xl shadow-2xl w-full max-w-sm pt-14 pb-8 px-8">
                    ...
                    flex items-center justify-center shadow-lg">
                            <span class="text-[#E8312A] font-black text-xl">Swapno</span>
                        </div>
                    </div>
                  ...
                  transition-all duration-200 mb-4">
                       ...
                       bg-white hover:bg-gray-50 transition-colors shrink-0">
                        ...
                        <input id="phoneInput" v-model="localPhone" type="tel" placeholder="1XXXXXXXXX"
